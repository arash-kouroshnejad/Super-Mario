    ...
    public SetupController(User usr) {
        ...
        for (Component btn : panel.getComponents()) {
            if (usrChars.contains(btn.getName())) {
                ...
                btn.setVisible(true);
                try {
                    ((JButton)btn).setIcon(new ImageIcon(ImageIO.read(new File(Config.getInstance().getProperty("SpritesDir") +btn.getName()+".png")).getScaledInstance(40, 40, Image.SCALE_DEFAULT))); // TODO : consolidate IO reads
                } catch (IOException e) {
